...
const d = JSON.parse(fs.readFileSync(new URL("./u4-parsed.json", import.meta.url)));
...
function needsCalc(q) {
  ...
  // scientific-notation arithmetic (universal gravitation etc.) — needs a calculator
  if ((blob.match(/\\times\s*10\^/g) || []).length >= 2) return true;
  // the gravitational constant appears in the working
  if (/6\.67\s*\\times\s*10/.test(blob)) return true;
  return false;
}
...
const SKILL = {
  "work": (arg) => (arg ? "misconception-diagnosis" : "construct-model"),
  "energy_definitions": (arg) => (arg ? "misconception-diagnosis" : "identify-principle"),
  "kinetic_energy": () => "construct-model",
  "gravitational_pe": () => "construct-model",
  "conservation": (arg) => (arg ? "misconception-diagnosis" : "justify-with-principle"),
  "spring_pe": () => "construct-model",
  "work_energy_theorem": (arg) => (arg ? "misconception-diagnosis" : "construct-model"),
  "gravity": (arg) => (arg ? "misconception-diagnosis" : "proportional-reasoning"),
  "inertia_and_n1": (arg) => (arg ? "misconception-diagnosis" : "justify-with-principle"),
  "force_identification_and_fbds": (arg) => (arg ? "misconception-diagnosis" : "construct-model"),
  "equilibrium": (arg) => (arg ? "misconception-diagnosis" : "justify-with-principle"),
  "newtons_second_law": () => "construct-model",
  "inclined_planes": () => "construct-model",
  "newtons_third_law": (arg) => (arg ? "misconception-diagnosis" : "identify-principle"),
  "hookes_law": (arg) => (arg ? "misconception-diagnosis" : "proportional-reasoning"),
...
const REP = {
  "work": "equation",
  "energy_definitions": "verbal",
  "kinetic_energy": "equation",
  "gravitational_pe": "equation",
  "conservation": "energy-bar-chart",
  "spring_pe": "equation",
  "work_energy_theorem": "equation",
  "gravity": "equation",
  "inertia_and_n1": "verbal",
  "force_identification_and_fbds": "free-body-diagram",
  "equilibrium": "free-body-diagram",
  "newtons_second_law": "free-body-diagram",
  "inclined_planes": "free-body-diagram",
  "newtons_third_law": "verbal",
  "hookes_law": "equation",
...
const HINT = {
  "work": ["Work $= F d$ only for the component of the force *along* the displacement. Force $\\perp$ displacement ($90^\\circ$) does zero work; force opposite the motion ($180^\\circ$) does negative work."],
  "energy_definitions": ["Mechanical energy is kinetic energy plus potential energy (gravitational + spring). Total energy also includes thermal and other forms."],
  "kinetic_energy": ["$KE = \\tfrac{1}{2}mv^2$. It depends on the *square* of the speed \u2014 doubling the speed quadruples the kinetic energy \u2014 and is never negative."],
  "gravitational_pe": ["$PE_g = mgh$, where $h$ is the height above a chosen reference level. Only *changes* in $PE_g$ are physically meaningful; the reference point is your choice."],
  "conservation": ["In an isolated (closed) system with no friction, mechanical energy is conserved: $KE + PE$ is the same at every point. Track the trade-off \u2014 as one goes up, the other goes down by the same amount."],
  "spring_pe": ["$PE_s = \\tfrac{1}{2}kx^2$, where $x$ is the stretch or compression from natural length. Like kinetic energy it depends on a *square*, so it is always non-negative."],
  "work_energy_theorem": ["The net work done on an object equals its change in kinetic energy: $W_{net} = \\Delta KE$. Positive net work speeds it up; negative net work slows it down."],
  "gravity": ["Gravity is always attractive, along the line joining the two centres of mass. In $F_G = G\\dfrac{m_1 m_2}{r^2}$, doubling a mass doubles $F_G$; doubling $r$ quarters it."],
  "inertia_and_n1": ["Newton's first law: with zero net force an object keeps a constant velocity (which may be zero). A change in the motion means the net force is not zero."],
  "force_identification_and_fbds": ["List only the forces actually acting ON the object: gravity from the Earth, and a contact force from each thing touching it. Net force along one axis is the signed sum."],
  "equilibrium": ["Equilibrium means the net force is zero, so the acceleration is zero \u2014 the velocity is constant. Static equilibrium is at rest; dynamic equilibrium is constant non-zero velocity."],
  "newtons_second_law": ["$F_{net} = ma$. Find the net force from the free-body diagram (signed sum along the axis), then divide by the mass. The acceleration points the same way as the net force."],
  "inclined_planes": ["Resolve along and perpendicular to the incline. The weight component along the slope is $mg\\sin\\theta$; perpendicular is $mg\\cos\\theta$. Friction (if any) opposes the motion or its tendency."],
  "newtons_third_law": ["A third-law pair is two forces of equal size and opposite direction that act on *two different objects*. Two forces on the *same* object that happen to balance are not a third-law pair."],
  "hookes_law": ["Hooke's law: $F = kx$, where $x$ is the extension from the natural length. On a force\u2013extension graph the slope is the spring constant $k$."],
...
const CAPTION = {
  "u4_energy_bar": "Energy bar chart for this item.",
  "u4_mcq_work": "Free-body / work diagram for this item.",
  "u4_gpe": "Height-and-reference diagram for this item.",
  "u4_spe": "Spring diagram for this item.",
  "u4_frq": "Diagram for this item.",
  "u3_fbd": "Free-body diagram for this item.",
  "u3_incline": "Inclined-plane diagram for this item.",
  "u3_spring": "Spring or force\u2013extension graph for this item.",
...
for (const q of d.mcq) {
  ...
  let stem = q.stem;
  if ((ck === "vector_components" || ck === "inclined_planes") && /3\s*7\^\\circ|5\s*3\^\\circ|37\^\\circ|53\^\\circ/.test(stem) && !/\\sin 37/.test(stem)) {
    stem = CONV + stem;
  }
  const incorrect = {};
  ...
  const item = {
    id: q.id.replace(/^U4-/, "BP8-U4-"),
    sourceId: q.id,
    courses: ["basis-physics-8"],
    topicId: "mechanical-energy",
    unit: "Unit 4: Energy",
    cluster: ck,
  ...
  if (q.asset) {
    ...
    item.figures = [{ svg: `basis-physics-8/unit-4/energy-bank/${name}`, caption: captionFor(q.asset) }];
  }
  mcqOut.push(item);
}
...
// SVGs transferred + restyled into assets/diagrams/basis-physics-8/unit-4/energy-bank/ (2026-09-03).
...
fs.writeFileSync(`${OUT_DIR}/mechanical-energy.json`, JSON.stringify(mcqOut, null, 2) + "\n");
fs.writeFileSync(`${OUT_DIR}/mechanical-energy-frq.json`, JSON.stringify(frqOut, null, 2) + "\n");
console.log("\nwrote mechanical-energy.json + mechanical-energy-frq.json");
