...
const SRC = ROOT + "resources/physics8/rescue_sprint/unit_04_energy/test_bank";
const PARSED_OUT = new URL("./u4-parsed.json", import.meta.url);
...
function splitItems(md, kind) {
  // kind: 'mcq' | 'frq'
  const re = kind === "mcq" ? /^### (U4-MCQ-\d+)\s*$/m : /^### (U4-FRQ-\d+)\s*$/m;
  const parts = md.split(/^### (U4-(?:MCQ|FRQ)-\d+)\s*$/m);
...
function parseMcq(id, body, clusterFile) {
  ...
  let afterAnswer = body.split(/\*\*Answer:\*\*.*\n/)[1] || "";
  // strip residual metadata lines that sit between the Answer line and the
  // question stem in some banks (U4 uses "**Numeric contract:** `...`" etc.).
  afterAnswer = afterAnswer.replace(/^\s*\*\*(Numeric contract|Difficulty basis|Estimated time|Difficulty|Misconception|Asset|Outcomes?):\*\*.*\n/gm, "");
  ...
  // rationale bullets: "- A: ..." / "- Correct (C): ..." / combined "- A, C, D: ..."
  const rats = {};
  let correctText = null;
  const ratRe = /^-\s+(?:\*\*)?(?:Correct\s*\(([A-E])\)|([A-E])|([A-E](?:\s*,\s*(?:and\s+)?[A-E])+))(?:\*\*)?\s*:\s*([\s\S]*?)(?=\n-\s|\n---|\n*$)/gm;
  let r;
  while ((r = ratRe.exec(rationaleBlock || ""))) {
    const t = r[4].replace(/\s+/g, " ").trim();
    if (r[1]) { correctText = t; rats[r[1]] = t; }
    else if (r[2]) { rats[r[2]] = t; }
    else if (r[3]) { for (const L of r[3].match(/[A-E]/g)) rats[L] = t; }
  }
...
console.log("wrote u4-parsed.json");
