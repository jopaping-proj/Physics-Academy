...
const SRC = ROOT + "resources/physics8/rescue_sprint/unit_03_forces/test_bank";
const PARSED_OUT = new URL("./u3-parsed.json", import.meta.url);
...
function splitItems(md, kind) {
  // kind: 'mcq' | 'frq'
  const re = kind === "mcq" ? /^### (U3-MCQ-\d+)\s*$/m : /^### (U3-FRQ-\d+)\s*$/m;
  const parts = md.split(/^### (U3-(?:MCQ|FRQ)-\d+)\s*$/m);
...
console.log("wrote u3-parsed.json");
