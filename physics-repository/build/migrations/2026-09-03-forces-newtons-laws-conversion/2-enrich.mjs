...
const d = JSON.parse(fs.readFileSync(new URL("./u3-parsed.json", import.meta.url)));
...
for (const q of d.mcq) {
  ...
  const item = {
    id: q.id.replace(/^U3-/, "BP8-U3-"),
    sourceId: q.id,
    courses: ["basis-physics-8"],
    topicId: "forces-and-newtons-laws",
    unit: "Unit 3: Forces and Newton's Laws",
    cluster: ck,
  ...
  if (q.asset) {
    ...
    item.figures = [{ svg: `basis-physics-8/unit-3/forces-bank/${name}`, caption: captionFor(q.asset) }];
  }
  mcqOut.push(item);
}
...
// SVGs transferred + restyled into assets/diagrams/basis-physics-8/unit-3/forces-bank/ (2026-09-03).
...
fs.writeFileSync(`${OUT_DIR}/forces-and-newtons-laws.json`, JSON.stringify(mcqOut, null, 2) + "\n");
fs.writeFileSync(`${OUT_DIR}/forces-and-newtons-laws-frq.json`, JSON.stringify(frqOut, null, 2) + "\n");
console.log("\nwrote forces-and-newtons-laws.json + forces-and-newtons-laws-frq.json");
