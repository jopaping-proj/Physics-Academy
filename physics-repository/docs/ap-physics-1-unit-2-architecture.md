...
12. **Concept-inventory gating + post entry point.** Currently a strongly-prompted homepage link, not a hard gate; the same page serves pre and post (`js/concept-inventory.js` treats attempt ≥ 2 as "post"). A unit index page should link the pre-check at unit start and the post-check at unit end.
...
15. **`objective` re-tag + `cedTopic` field.** Re-tag the ~65 bank items and the prototype lesson's embedded questions from bare `"2.1"…"2.10"` to the `C`-prefixed cluster form, and add a `cedTopic` field (e.g. `"2.5"`) per the §2 map. Fold into the §12.6 ID migration. (Lesson 2's items already use CED topic `"2.2"` in `objective` — reconcile.)
