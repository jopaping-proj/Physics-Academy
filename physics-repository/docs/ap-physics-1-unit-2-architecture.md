...
*Resolved from "optional" in the CED-verification pass — circular motion is CED Unit 2 topic 2.9. Delivered as Lesson 11 in §5 (`circular-motion.json`, built 0.9.11). Bank `ap1-u2-circular.json` (`AP1-U2-MCQ-107…112`).*
...
### Lesson 8 — Connected Objects and Systems · Cluster 2.7  *(built — approved 2026-09-02)*
...
### Lesson 9 — Gravitation, Springs, and Apparent Weight · Cluster 2.10  *(built — approved 2026-09-02)*
...
**Built as** `gravitation-springs-apparent-weight.json` (`ap1-u2-grav-spring-aw`, lessonNumber 9, C2.10; per-item `cedTopic` 2.6 / 2.8 / 2.5 by strand). New **Spring Force–Extension Graph** interactive (`spring-graph-explorer`, on `PA.panel` — slope = k, predict F at any x). Diagrams: hand-authored `gravitation/two-bodies.svg`, `springs/spring-cases.svg`; generated `apparent-weight/dot-accel-up.svg` + `dot-accel-down.svg`. Data-table graph-and-justify FRQ; velocity-vs-acceleration-sign error analysis; bank `ap1-u2-grav-spring-aw.json` (`AP1-U2-MCQ-095…100`). Four new misconception slugs added to `data/taxonomies.json` (`g-and-G-are-the-same-quantity`, `spring-constant-changes-with-applied-force`, `scale-always-reads-true-weight`, `apparent-weightlessness-requires-zero-gravity`).

### Lesson 10 — Inclined Planes · Cluster 2.9  *(synthesis capstone)* — BUILT — approved 2026-09-02

**Built as** `inclined-planes.json` (`ap1-u2-inclines`, lessonNumber 10, `objective` C2.9 / `cedTopic` 2.5 — CED-topic-2.5 application). 3 chunks: frictionless incline (tilt axes, resolve weight, $F_N = mg\cos\theta < mg$, $a = g\sin\theta$, trig assignment justified by the $\theta\to0$/$\theta\to90°$ limits) / incline with friction ($a = g(\sin\theta - \mu_k\cos\theta)$, slide-or-stay via $\tan\theta = \mu_s$) / the incline in disguise (frictionless banked turn $\tan\theta = v^2/rg$, accelerating wedge $a = g\tan\theta$ — axis choice follows the acceleration direction). Interactive reuses `componentKey: "fbd-builder"` (the two 45° ramp scenarios). Diagrams: hand-authored `incline/incline-frictionless.svg` + `incline/incline-friction.svg` (both `size: "lg"`); generated `incline/dot-frictionless.svg` + `incline/dot-friction.svg`. Level-6 multi-part FRQ (worker pushes a crate up a ramp, applied force parallel to the surface — FBD → $F_N$ → friction → $a$); error analysis on $F_N = mg$ vs $mg\cos\theta$ for the friction term; two misconceptions (`incline-weight-components-sine-cosine-swapped`, `steeper-incline-always-means-faster-final-speed`); `distinction-stretch` far-transfer MCQ (bead in a smooth conical bowl). Bank `ap1-u2-inclines.json` (`AP1-U2-MCQ-101…106`).
...
### Lesson 11 — Circular Motion · C2.12 (CED topic 2.9) — BUILT — approved 2026-09-02

**Built as** `circular-motion.json` (`ap1-u2-circular`, lessonNumber 11, `objective` C2.12 / `cedTopic` 2.9). 3 chunks: turning-is-accelerating ($a_c = v^2/r$ toward the centre; velocity tangent, acceleration radial, perpendicular) / real forces do it ($\sum F_r = mv^2/r$; "centripetal force" is not an FBD arrow — tension/friction/normal-component/gravity play the role) / limiting cases ($v_{\max} = \sqrt{\mu_s g r}$ on an unbanked curve; the snapped string → tangent line, Newton's first law). New **Circular Motion Explorer** interactive (`circular-motion-explorer`, on `PA.panel`; an animated top-view puck-on-a-string — velocity tangent, acceleration radial, string tension = $F_c$, snaps and flies off tangentially when $mv^2/r$ exceeds the set breaking tension). Diagrams: hand-authored `circular/uniform-circle.svg` (`size: "lg"`) + `circular/string-break.svg` (`size: "lg"`); generated `circular/dot-car-curve.svg`. Level-5 multi-part FRQ (puck on a string: FBD → tension → $v_{\max}$ → describe motion after the break), fake-"centrifugal-force"-equilibrium error analysis, `distinction-stretch` transfer MCQ (passenger pressed against a car door). Two misconceptions (`objects-in-circular-or-turning-motion-feel-an-outward-force`, `centripetal-force-is-a-separate-force` — both already in `data/taxonomies.json`). Bank `ap1-u2-circular.json` (`AP1-U2-MCQ-107…112`). Boundary respected: uniform circular motion only; the vertical-circle items use given speeds, no energy analysis.
...
|---|---|---|---|
| 0.9.21 | 2026-09-02 | Draft — for review | **Lesson 11 & 12 approved — AP Physics 1 Unit 2 is COMPLETE.** `circular-motion.json` and `unit-2-synthesis.json` `status` → approved; `unit-2-index.json` chips updated. **All 12 Unit 2 lessons + the concept inventory are now approved.** The unit test (`unit-2-test.json`, 155 MCQ + 25 FRQ pool) and the linked-header / end-of-lesson-button infrastructure are course-neutral and ready for the next unit. Physics self-audit of the Lesson 11 Cutnell & Johnson derivation and `FRQ-016…025` is in `docs/reviews/2026-09-02-lesson-11-and-new-frqs.md` (one fix: `FRQ-020` dataset). The Lesson 11 la02 `circular/fbd-puck-2view.svg` now shows the string tension pointing toward the centre consistently in the top and side views, with the §11 box-diagram anchoring. |
| 0.9.20 | 2026-09-02 | Draft — for review | **Lesson 10 approved; Lesson 11/12 diagram fixes.** `inclined-planes.json` `status` → approved — AP Physics 1 Unit 2 now has **ten approved lessons (1–10)**; 11–12 draft. **Lesson 11:** the centripetal-derivation diagram now renders at the **top** of Concept 1's representation card (new `chunk.figuresFirst` flag in `build/render/sections.js`); the rider-at-the-dip example diagram (`circular/ex-dip.svg`) redrawn so both force arrows start at the dot. **Lesson 12:** Concept 3's worked example gains an FBD (`incline/fbd-rope-above-surface.svg`, rope $20^\circ$ above the surface); the lesson-check `la04` diagram (`incline/fbd-drag-up-pulley.svg`) rewritten to follow the §11 box-diagram rules — contact forces ($F_N$, $f_k$) from the contact face, tension from the box, gravity from the centre, box flush on the incline. **`docs/test-generation.md`** gains §4a "Adding a unit test to a new unit" — the checklist and the current blocker (no native question banks for AP Physics 2 / IB / BASIS Physics 8; only `BP8-*` provenance manifests exist). |
| 0.9.19 | 2026-09-02 | Draft — for review | **Test-bank depth: recall MCQs + 10 new FRQs.** New `data/question-bank/ap1-u2-exam-mix-lo.json` (`AP1-U2-MCQ-148…155`) — 8 cognitiveLevel-1–2 recall/direct-application items, lifting the pool's band-A cognitive share from ~19 % to ~23 %. New `data/question-bank/ap1-u2-frq-extra.json` (`AP1-U2-FRQ-016…025`) — 10 FRQs across inclines (2, incl. an experimental-design item for $\mu_s = \tan\theta_c$), circular motion (2, incl. a vertical-circle tension item), springs / apparent weight (2), connected systems (2, incl. an error-analysis item), Newton's third law (1 paragraph-argument), and a multi-force synthesis capstone. Both banks added to `unit-2-test.json`. AP U2 test-bank pool is now **155 MCQ + 25 FRQ**. All difficulty buckets within blueprint tolerance; no `build/render/unit-test.js` pool warnings. |
| 0.9.18 | 2026-09-02 | Draft — for review | **Lesson 11 centripetal derivation + example diagrams; unit-test LaTeX; linked page headers; Lesson 10 & 12 diagram fixes.** **Lesson 11:** Concept 1 gains a `representation` card deriving $a_c = v^2/r$ by the similar-triangles argument of *Cutnell & Johnson, Physics* §5.2, with a new two-panel `circular/centripetal-derivation.svg` (velocity triangle + position triangle). Concept 2's four "which real force is centripetal" examples each get a diagram (`circular/ex-string.svg`, `ex-flat.svg`, `ex-banked.svg`, `ex-dip.svg`); the Concept-2 formative check draws the four candidate FBDs (`circular/q02-options.svg`) with the choices reworded to "Diagram A…D"; the lesson-check FRQ `la02` part (a) model response shows `circular/fbd-puck-2view.svg`. Chunk 1 & 2 titles reworded to plain text (`.slide__title`'s `text-transform: uppercase` mangles KaTeX in a title). **Lesson 10:** `incline-frictionless.svg` "$mg\cos\theta$" label moved next to its component arrow. **Lesson 12:** `la04` set-up figure and part (a) model response now use `incline/fbd-drag-up-pulley.svg` (box dragged *up* ⇒ $f_k$ points *down* the slope) instead of the sliding-down diagram. **Unit-test LaTeX:** `ap1-u2-dynamics.json` (50), `ap1-u2-forces-fbd.json` (8) and `ap1-u2-dynamics-frq.json` (15) rewritten so every value+unit, variable=value, angle and trig term is a `$…$` expression, matching the newer banks. **Linked page headers:** new `linkedBreadcrumb()` in `build/build.js` — the concept-inventory, unit-test, unit-index and external-html page headers now link course → homepage and unit → the unit index, like the lesson pages. |
| 0.9.17 | 2026-09-02 | Draft — for review | **Lesson 9 approved; incline angle re-marked; higher-order MCQ depth.** `gravitation-springs-apparent-weight.json` `status` → approved; `unit-2-index.json` Lesson 9 chip → approved. AP Physics 1 Unit 2 now has **nine approved lessons (1–9)**; 10–12 draft. `incline/incline-frictionless.svg` + `incline/incline-friction.svg`: the θ marking is now the angle **between the weight vector and the into-slope perpendicular direction (opposite F_N)**, drawn **below the ramp surface** where the wedge has room — the arc and label sit in the open shaded-triangle area, clear of the arrows and the completing parallelogram. New `data/question-bank/ap1-u2-exam-mix-hi.json` (`AP1-U2-MCQ-133…147`) — 15 level-5–8 transfer/synthesis MCQs across all Unit 2 clusters, closing the cognitive band-C gap flagged in 0.9.16. AP U2 MCQ pool now 147; cognitive spread A/B/C now closer to the 25/40/35 blueprint target. |
| 0.9.16 | 2026-09-02 | Draft — for review | **Diagram label fixes + test-bank depth (Step 3).** Lesson 9: `springs/force-extension-graph.svg` "elastic limit" label moved above-left of the point with a leader (was crossing the curve); `springs/force-extension-q02.svg` "(0.05 m, 4.0 N)" label moved below-right with a leader (was on the line). Lesson 10: `incline/incline-frictionless.svg` — the θ marking is now between the normal force and a dashed vertical reference line up top (F_N ⟂ slope ⇒ that angle is θ), replacing the cramped θ that overlapped the perpendicular weight component near the block; F_N shortened slightly to clear the angle zone. **Test-bank depth:** new `data/question-bank/ap1-u2-exam-mix.json` (`AP1-U2-MCQ-119…132`) — 14 items across all Unit 2 clusters, deliberately filling the thin blueprint buckets: `foundation`/`developing` at cognitiveLevel 1–2 (recall/application band) and `ap5-ib7-target` at level 6 (transfer/synthesis band); all `calculatorFree: true`. Added to `unit-2-test.json`'s `mcqBanks`. AP U2 MCQ pool now 132 items: difficulty foundation 12 / developing 16 / standard 34 / target 31 / stretch 7 % (all within the ±12 pp blueprint tolerance — `build/render/unit-test.js` prints no pool warnings). **Residual gap:** cognitively the pool still leans band B (conceptual, 54 %) over band C (higher-order, 25 % vs 35 % target) — a deeper authoring task, tracked. |
| 0.9.15 | 2026-09-02 | Draft — for review | **Test generator + Lesson 9 & 10 review rounds.** **Test generator** (`docs/test-generation.md`): reviewed `resources/physics8/test_generator` and rebuilt its logic for native JSON banks + in-browser delivery. New `data/test-blueprint.json` — the canonical composition target for every test in every course: 50/50 MCQ/FRQ points, a difficulty distribution one band harder than the real AP/IB exam (median `ap-ib-standard`, 35 % at/above the AP-5/IB-7 line — "train above target"), cognitive bands, AP-2025 timing constants, calculator policy by course (IB defaults to `not-allowed` per owner directive; doc notes the 2025 IB syllabus now permits calculators on Paper 1A). New `format: "unit-test"` — `build/render/unit-test.js` (assembly-time pool build + blueprint adequacy warnings), `build/templates/unit-test.html`, `js/unit-test.js` (attempt-time stratified draw from a fresh seed every visit, shuffled question + option order, countdown auto-submit, MCQ auto-scored, FRQ self-scored vs rubric, composite vs 60 % pass / 90 % target, `pa:ut:<id>` history). `config` supports `mode: "time"` (minutes → counts via the timing constants) or `mode: "count"`. Reference: `content/ap-physics-1/unit-2-dynamics/unit-2-test.json` (all 12 lesson MCQ banks + the FRQ bank; 45 min → ~15 MCQ + 2 FRQ). Registered in `unit-2-index.json` after the post concept-check. `build/render/primitives.js` `renderFormativeCheck` now pre-renders an FRQ `scenario` containing a markdown table to `scenarioHtml` (`js/assessment.js` uses it) — fixes the raw-pipe data table in Lesson 9's `la02`. **Lesson 9 review round:** chunk-2 gained Hooke's-law + elastic-limit content and stronger force–extension graph-reading, a problem diagram on the worked example, a graph on the formative check, and the Spring Graph Explorer now sits right after chunk-2 (new `interactiveComponent.afterChunk`); chunk-3 gained a scale-in-an-elevator diagram and the acceleration direction is now marked on the elevator dot-FBDs (new `spec.accel` in `build/render/fbd-svg.js`). New SVGs: `springs/hanging-spring-stretch.svg`, `springs/force-extension-graph.svg`, `springs/force-extension-q02.svg`, `apparent-weight/scale-in-elevator.svg`. **Lesson 10 review round:** `incline/incline-frictionless.svg` + `incline/incline-friction.svg` rewritten with correct trig — the block sits flush on the ramp, contact forces start at the contact face (gravity still from the centre), and the frictionless diagram marks the angle between the weight and the perpendicular component (= θ); dot-diagram hints added to misconception 1, the lesson-check FRQ part (a) model response, and bank items 101–104. |
| 0.9.14 | 2026-09-02 | Draft — for review | **Lesson 8 approved.** `connected-objects-and-systems.json` (`ap1-u2-connected`) `status` → approved (physics reviewed 2026-09-02); `unit-2-index.json` Lesson 8 chip → `approved`. AP Physics 1 Unit 2 now has eight approved lessons (1–8); Lessons 9–12 draft. |
| 0.9.13 | 2026-09-02 | Draft — for review | **BASIS Physics 8 template gaps closed.** New `docs/basis-physics-8-architecture.md` (v0.1.0) — a lightweight structural map for the second course: unit list, outcome-source provenance (`resources/physics8/resources/` only), lesson format (same JSON schema as AP), test-bank provenance (the `BP8-*` manifests are ledgers, not native arrays), and an open-items list. Records that BASIS Physics 8 reuses the AP build / template / validator / interactive scaffolding unchanged and deliberately does not adopt the `objective`/`cedTopic` scheme. **Progressive disclosure** (`hint` array + `solution` string, driving `js/assessment.js` `renderProgressiveDisclosure`) added to all five question items of the one built BASIS lesson (`motion-language-reference-frames.json` — 2 formative checks, 1 misconception, 1 lesson-assessment, 1 exit question), matching the AP lesson template. Deferred with rationale: native `BP8-*` question-bank conversion (large); Unit 1 real outcome source; a second built BASIS lesson. This resolves Step 4 of the "Resolve Steps 3–5" work. |
| 0.9.12 | 2026-09-02 | Draft — for review | **Lesson 12 built — Unit 2 Synthesis and Transfer. AP Physics 1 Unit 2 lesson sequence now complete (all 12 built; Lessons 1–7 approved, 8–12 draft).** `unit-2-synthesis.json` (`ap1-u2-synthesis`, lessonNumber 12, no lesson-level `objective`): 4 interleaved chunks — recognise-the-four-types (classification table + reasonableness check) / interaction task / uncued multi-force task (incline + angled rope) / force–extension graph task. Reuses `fbd-builder`. Vertical-circle fake-equilibrium error analysis; Level-6 multi-part FRQ (incline + pulley + hanging mass, released $a \approx 0$ — a sign-interpretation test); 4-way reasonableness-check exit question; two cross-cutting misconceptions (all slugs already in `data/taxonomies.json`). Bank `ap1-u2-synthesis.json` (`AP1-U2-MCQ-113…118`, interleaved). No new diagrams. Registered in `unit-2-index.json` (Lesson 12 → slug `unit-2-synthesis`, status draft). Build + Playwright verified: 0 validate warnings, 0 KaTeX errors, 0 JS errors; markdown table renders with inline KaTeX. |
| 0.9.11 | 2026-09-02 | Draft — for review | **Lesson 11 built — Circular Motion.** `circular-motion.json` (`ap1-u2-circular`, lessonNumber 11, `objective` C2.12 / `cedTopic` 2.9): 3 chunks — turning-is-accelerating ($a_c = v^2/r$, velocity tangent / acceleration radial) / real forces do it ($\sum F_r = mv^2/r$; no "centripetal force" FBD arrow) / limiting cases ($v_{\max} = \sqrt{\mu_s g r}$; snapped string → tangent line). New **Circular Motion Explorer** interactive (`circular-motion-explorer`, `js/lesson-interactives/circular-motion-explorer.js`, registered in `COMPONENT_SCRIPTS` — an animated top-view puck-on-a-string; velocity tangent, acceleration radial, tension = $F_c$, snaps and flies off tangentially past the breaking tension). New diagrams: hand-authored `circular/uniform-circle.svg` + `circular/string-break.svg` (both `size: "lg"`); generated `circular/dot-car-curve.svg` (added to `build/gen-diagrams.js`). Level-5 multi-part FRQ, fake-"centrifugal-force" error analysis, `distinction-stretch` transfer MCQ. Bank `ap1-u2-circular.json` (`AP1-U2-MCQ-107…112`). Both misconception slugs were already in `data/taxonomies.json`. Registered in `unit-2-index.json` (Lesson 11 → slug `circular-motion`, status draft). Build + Playwright verified: 0 validate warnings, 0 KaTeX errors, 0 JS errors. Only Lesson 12 (Unit 2 Synthesis) remains to build. |
| 0.9.10 | 2026-09-02 | Draft — for review | **Lesson 10 built — Inclined Planes (synthesis capstone).** `inclined-planes.json` (`ap1-u2-inclines`, lessonNumber 10, `objective` C2.9 / `cedTopic` 2.5): 3 chunks — frictionless incline (tilt axes; $F_N = mg\cos\theta < mg$; $a = g\sin\theta$; trig assignment justified by $\theta\to0$/$\theta\to90°$ limits) / friction ($a = g(\sin\theta - \mu_k\cos\theta)$; slide-or-stay via $\tan\theta = \mu_s$) / transfer (banked turn $\tan\theta = v^2/rg$; accelerating wedge $a = g\tan\theta$; axis choice follows the acceleration direction). Interactive reuses `componentKey: "fbd-builder"`. New diagrams: hand-authored `incline/incline-frictionless.svg` + `incline/incline-friction.svg` (`size: "lg"`); generated `incline/dot-frictionless.svg` + `incline/dot-friction.svg` (added to `build/gen-diagrams.js`). Level-6 multi-part FRQ (crate pushed up a ramp), `F_N = mg` vs `mg\cos\theta` error analysis, `distinction-stretch` far-transfer MCQ (bead in a conical bowl), 2 misconceptions (`incline-weight-components-sine-cosine-swapped`, `steeper-incline-always-means-faster-final-speed`). Bank `ap1-u2-inclines.json` (`AP1-U2-MCQ-101…106`). Registered in `unit-2-index.json` (Lesson 10 → slug `inclined-planes`, status draft). Build + Playwright verified: 0 validate warnings, 0 KaTeX errors, 0 JS errors. |
| 0.9.9 | 2026-09-02 | Draft — for review | **Lesson 9 built; Lesson 8 review round; sidebar navigation.** Lesson 9 — Gravitation, Springs, and Apparent Weight (C2.10) — `gravitation-springs-apparent-weight.json`: 3 strands (gravitation / springs / apparent weight), new **Spring Graph Explorer** (`spring-graph-explorer`), 4 diagrams, a graph-and-justify FRQ, bank `ap1-u2-grav-spring-aw.json` (`AP1-U2-MCQ-095…100`), 4 misconception slugs added. **Lesson 8** review-round diagrams: worked-example / hint / model-response figures added throughout (new `connected/two-blocks-pulled.svg`, `cart-trailer.svg` with wheels on the track line, `system-then-isolate.svg` two-panel scenario→isolation, and dot FBDs `dot-table-block` / `dot-hanging-block` / `dot-elevator-bag`). **Sidebar navigation:** the left-column breadcrumb is now linked — course → homepage, unit → that unit's index page (the sibling `*-index.json`), dotted-underline styling. Note: `·` (U+00B7) inside a KaTeX `\text{}` throws — use `\mathrm{…\,…}` for unit strings. |
