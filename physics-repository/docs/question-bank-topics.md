...
**Version:** 0.2.0 · 2026-09-06
**Status:** Draft — for review. Introduces `topicId` and the first shared bank (`kinematics-1d`); registry now covers all BASIS Physics 8 Units 2–8 topics (`thermal-physics`, `momentum-and-collisions`, `geometric-optics`, `dc-circuits` added as hand-authored tranche-1 banks).
...
| `momentum-and-collisions` | `momentum-and-collisions.json`, `momentum-and-collisions-frq.json` | `basis-physics-8` | `ap-physics-1`, `ap-physics-2`, `ib-physics-sl`, `ib-physics-hl` | Linear momentum ($p = mv$); impulse and the impulse–momentum theorem ($J = F\Delta t = \Delta p$), including force–time graphs; conservation of momentum for an isolated system; elastic vs inelastic collisions in 1-D; recoil and the ballistic pendulum. 1-D only, right = positive; no 2-D momentum, no centre-of-mass frame. Hand-authored (no `rescue_sprint` source for Unit 6) — 54 MCQ (all `calculatorFree`) + 14 FRQ, calibrated against the AP Physics 1 CED Unit 4 and OpenStax HS Physics Ch. 8. Tranche 1. |
| `geometric-optics` | `geometric-optics.json`, `geometric-optics-frq.json` | `basis-physics-8` | `ap-physics-2`, `ib-physics-sl`, `ib-physics-hl` | Nature of light and basic definitions; law of reflection; plane and spherical (concave / convex) mirror images by ray diagram; index of refraction ($n = c/v$); Snell's law solved for an index only; total internal reflection ($\sin\theta_c = 1/n$); dispersion and diffraction qualitatively. Ray diagrams only — no thin-lens equation, no magnification algebra, no two-source interference. Hand-authored (no `rescue_sprint` source for Unit 7) — 57 MCQ (all `calculatorFree`) + 14 FRQ, calibrated against the AP Physics 2 CED Unit 13 and OpenStax HS Physics Ch. 16. Tranche 1. |
| `dc-circuits` | `dc-circuits.json`, `dc-circuits-frq.json` | `basis-physics-8` | `ap-physics-2`, `ib-physics-sl`, `ib-physics-hl` | Charge, current ($I = Q/t$) and voltage; resistance and its dependence on wire length / thickness; Ohm's law ($V = IR$); circuit-diagram symbols (battery, bulb, resistor, switch); VIR charts for three-or-fewer components; equivalent resistance in series ($R_s = \Sigma R$) and parallel ($1/R_p = \Sigma 1/R$). DC analysis only — no electric-power formula, EMF / internal resistance, capacitors, or Kirchhoff simultaneous equations. Hand-authored (no `rescue_sprint` source for Unit 8) — 56 MCQ (all `calculatorFree`; parallel pairs chosen for whole-number results) + 14 FRQ, calibrated against the AP Physics 2 CED Unit 11 and OpenStax HS Physics Ch. 18–19. Tranche 1. |
