...
| 3 | Forces and Newton's Laws | `resources/physics8/resources/unit3_forces_newtonslaws_outcomes.md` | `unit-3-index.json` | **L1 Gravity** (`gravity.json`, **draft**); native bank `forces-and-newtons-laws.json` (250 MCQ) + `-frq.json` (50 FRQ) converted from the Founder-approved `rescue_sprint/unit_03_forces/test_bank/`; **Unit Test** (`unit-3-test.json`, no-calculator, **draft**). L2–L9 planned. |
...
1. **Native question banks.** Units 2 (`kinematics-1d`) and 3 (`forces-and-newtons-laws`) fully converted, 250 MCQ + 50 FRQ each, SVGs transferred (§4). Remaining: convert Units 4–8; build Unit 3 Lessons 2–9; independent physics review of both banks; promote `topicId` to a controlled vocabulary in `data/taxonomies.json` (blocked while a parallel session holds edits there); cross-tag to AP/IB once those units are built.
